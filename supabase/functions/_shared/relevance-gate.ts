/**
 * The relevance gate: decides whether a user→assistant exchange from a
 * Claude Code session is worth saving. Used by the Stop hook and the mine
 * CLI through the Node binding (lib/relevance-gate.ts); lives here so the
 * prompt sits with the other prompts and any runtime can run the gate
 * through the model-call seam.
 */
...
export async function relevanceGate(ai: Ai, input: GateInput): Promise<GateResult> {
	const req = {
	...
	try {
		const { text, usage } = ai.generateWithUsage
			? await ai.generateWithUsage(req)
			: { text: await ai.generate(req), usage: { inputTokens: 0, outputTokens: 0 } };
