...
import { compileEntityPage, getEmbedding } from "./ai.ts";
...
async function getRelatedEntities(entityId: string): Promise<RelatedEntity[]> {
 ...
 */
export async function recompileEntityPage(entityId: string): Promise<{
	title: string;
	entity_type: string;
	...
	if (thoughtIds.length < CREATION_THRESHOLD) {
		...
		await supabase.from("entity_pages").delete().eq("entity_id", entityId);
		return null;
	}
	...
	const summary = await compileEntityPage(entity.canonical_name, entity.type, thoughts, related);
	const embedding = await getEmbedding(`${entity.canonical_name}\n\n${summary}`);
	const compiledIds = (thoughtRows ?? []).map((t: { id: string }) => t.id);

	await supabase.from("entity_pages").upsert(
		{
			entity_id: entityId,
 ...
 */
export async function updateEntityPagesForThought(entityIds: string[]): Promise<void> {
	...
	try {
		...
		for (const row of (rows ?? []) as { id: string }[]) {
			await recompileEntityPage(row.id).catch((e) =>
