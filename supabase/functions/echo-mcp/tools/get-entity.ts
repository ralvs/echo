	...
	server.registerTool(
		...
		async ({ id, name, type }) => {
			try {
				...
				if (error || !entity) {
					return {
						content: [{ type: "text" as const, text: `Entity not found: ${id ?? name}` }],
						isError: true,
					};
				}
				...
				if (entity.aliases?.length)
					parts.push(`Aliases: ${(entity.aliases as string[]).join(", ")}`);
				...
				if (thoughtIds.length) {
					...
					if (thoughts?.length) {
						const lines = thoughts.map((t: { content: string; created_at: string }) => {
