-- Fix people aggregation in get_thought_stats() to resolve via the people table.
-- Previously read raw metadata->'people' which surfaced role-based terms
-- ("daughter", "mother-in-law") alongside canonical names.
-- Now joins with the people table: only canonical names appear, and any
-- alias still present in thought metadata is collapsed to its canonical name.
...
BEGIN
  ...
  -- Only count people that exist in the people table.
  -- Aliases in thought metadata are resolved to their canonical name so
  -- stale alias entries and canonical entries are not double-counted.
  SELECT json_object_agg(canonical_name, cnt)
  INTO people_json
  FROM (
    SELECT p.canonical_name, COUNT(*) AS cnt
    FROM thoughts t,
         jsonb_array_elements_text(COALESCE(t.metadata->'people', '[]'::jsonb)) AS person
    JOIN people p ON person = p.canonical_name OR person = ANY(p.aliases)
